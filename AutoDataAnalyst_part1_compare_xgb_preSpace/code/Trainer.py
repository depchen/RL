...
from RL.AutoDataAnalyst_part1_compare_xgb_preSpace.code import DataManager
from RL.AutoDataAnalyst_part1_compare_xgb_preSpace.code.configFile.ClassificationAlgorithmConfigureFile import ClassificationAlgorithmConfigure
...
class Trainer(object):
    def __init__(self, actions, c_config, c_i, data_manager):
        ...
        if c_i == 0:
            self.method = XGBClassifier(max_depth=param[0], learning_rate=param[1], n_estimators=param[2], gamma=param[3], min_child_weight=param[4],
                                        subsample=param[5], colsample_bytree=param[6], colsample_bylevel=param[7], reg_alpha=param[8], reg_lambda=param[9],
                                        nthread=-1)
        elif c_i == 1:
            self.method = RandomForestClassifier(n_estimators=param[0], max_depth=param[1], min_samples_split=param[2],
                                                 min_samples_leaf=param[3], max_features=param[4], bootstrap=param[5],
                                                 n_jobs=-1)
        else:
