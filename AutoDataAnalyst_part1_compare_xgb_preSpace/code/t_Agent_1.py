...
from RL.AutoDataAnalyst_part1_compare_xgb_preSpace.code.configFile.AgentConfigFile import AgentConfig as Config
...
class LSTM:
    ...
    def __init__(self, params):
        ...
        with tf.name_scope('train'):
            ...
            #self.train_op=AddSign(Config.lr,self.jj).minimize(self.loss)
    ...
    def learn(self, sess, x, labels, rewards, baseline_reward, jj, writer):
    ...
    def getInput(self, kk, init_input_c, c_config):
        ...
        #n_step有几个超参数
        for i in range(self.n_step):
            x1.append(init_input)
            init_input = []
            for j in range(Config.batch_size):
                #c_config.config_list[kk][i]  代表第kk个算法下第i个超参数下有多少个预选值
                #var_p 构建一个样本的one-hot  假如有3个预选值 则根据 top_agr_params中的index值
                # top_agr_params 是  8*5  每一列代表一个超参数的样本最优值
                #init_input 是一个 8*3 的one-hot值
        ...
        init_input = init_input_c
        for i in range(self.n_step):
            x2.append(init_input)
            init_input = []
            for jj in range(8,16):
                var_p = np.zeros(c_config.config_list[kk][i])
                var_p[top_agr_params[:, i][jj]] = 1
                init_input.append(var_p)
            init_input = np.array(init_input).reshape(Config.batch_size, c_config.config_list[kk][i])
            # 把最后一个差参数的输出赋值给第一个x
            # if (i == self.n_step - 1):
            #     x[0] = init_input
        #返回最优的并且是完整的  x
        return x1,x2, top_agr_params, top_rewards
    ...
    def getdata(self, output):
        ...
        #P1 是batch_size= 8    p2 =12第i个超参数有多少个预选值
        (p1, p2) = output.shape
        agr_params = []
        params = []
        #循环batch_size中的每个样本
        for k in range(p1):
            #从range(p2) 即0-p2-1 范围中  按 p 概率选择一个(自然选择最大的那个概率的 index)
            ...
            var_p[p] = 1
            params.append(var_p)
        #当前时刻的标签 lable 8*1,   每个数代表当前超参数 在每个样本中最大概率的 index
        agr_params = np.reshape(np.array(agr_params), [p1, 1])
        #当前时刻的输入 one-hot类型 作为下一个时刻输入   每个超参数在batch_size中的每个样本中的onehot,index为1,其他为0
    ...
    for i in range(1):
        ...
        with tf.Session() as sess:
            ...
            for j in range(200):
                ...
                with open(file_name, 'a') as f:
                    f.write("\n i = " + str(i) + ", j = " + str(j) + "\n")
                #给init_input输入   输出:当前时刻 标签agr_params   输出 init_input(下一时刻输入)
